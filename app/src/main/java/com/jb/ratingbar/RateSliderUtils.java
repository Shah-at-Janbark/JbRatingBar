...
class RateSliderUtils {
    ...
    static float calculateRating(SliderElement sliderElement, float stepSize, float eventX) {
        DecimalFormat decimalFormat = RateSliderUtils.getDecimalFormat();
        float ratioOfView = Float.parseFloat(decimalFormat.format((eventX - sliderElement.getLeft()) / sliderElement.getWidth()));
        float steps = Math.round(ratioOfView / stepSize) * stepSize;
        return Float.parseFloat(decimalFormat.format((int) sliderElement.getTag() - (1 - steps)));
    }
