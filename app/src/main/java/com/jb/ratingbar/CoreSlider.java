...
public class CoreSlider extends LinearLayout implements RatingSliderBar {

    public interface OnRatingChangeListener {
        void onRatingChange(CoreSlider ratingBar, float rating, boolean fromUser);
    }
    ...
    protected List<SliderElement> mSliderElements;

    public CoreSlider(Context context) {
        this(context, null);
    }

    /* Call by xml layout */
    public CoreSlider(Context context, @Nullable AttributeSet attrs) {
     ...
     */
    public CoreSlider(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void verifyParamsValue() {
        ...
        mMinimumStars = RateSliderUtils.getValidMinimumStars(mMinimumStars, mNumStars, mStepSize);
    }

    private void initRatingView() {
        mSliderElements = new ArrayList<>();

        for (int i = 1; i <= mNumStars; i++) {
            SliderElement sliderElement = getPartialView(i, mStarWidth, mStarHeight, mPadding, mFilledDrawable, mEmptyDrawable);
            addView(sliderElement);

            mSliderElements.add(sliderElement);
        }
    }

    private SliderElement getPartialView(final int partialViewId, int starWidth, int starHeight, int padding,
                                         Drawable filledDrawable, Drawable emptyDrawable) {
        SliderElement sliderElement = new SliderElement(getContext(), partialViewId, starWidth, starHeight, padding);
        sliderElement.setFilledDrawable(filledDrawable);
        sliderElement.setEmptyDrawable(emptyDrawable);
    ...
    protected void fillRatingBar(final float rating) {
        for (SliderElement sliderElement : mSliderElements) {
            int ratingViewId = (int) sliderElement.getTag();
    ...
    public void setStarWidth(@IntRange(from = 0) int starWidth) {
        ...
        for (SliderElement sliderElement : mSliderElements) {
            sliderElement.setStarWidth(starWidth);
        }
    }
    ...
    public void setStarHeight(@IntRange(from = 0) int starHeight) {
        ...
        for (SliderElement sliderElement : mSliderElements) {
            sliderElement.setStarHeight(starHeight);
        }
    }
    ...
    public void setStarPadding(int ratingPadding) {
        ...
        for (SliderElement sliderElement : mSliderElements) {
            sliderElement.setPadding(mPadding, mPadding, mPadding, mPadding);
        }
    }
    ...
    public void setEmptyDrawable(@NonNull Drawable drawable) {
        ...
        for (SliderElement sliderElement : mSliderElements) {
            sliderElement.setEmptyDrawable(drawable);
        }
    }
    ...
    public void setFilledDrawable(@NonNull Drawable drawable) {
        ...
        for (SliderElement sliderElement : mSliderElements) {
            sliderElement.setFilledDrawable(drawable);
        }
    }

    @Override
    public void setMinimumStars(@FloatRange(from = 0.0) float minimumStars) {
        mMinimumStars = RateSliderUtils.getValidMinimumStars(minimumStars, mNumStars, mStepSize);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (!RateSliderUtils.isClickEvent(mStartX, mStartY, event) || !isClickable()) {
                    return false;
                }
    ...
    private void handleMoveEvent(float eventX) {
        for (SliderElement sliderElement : mSliderElements) {
            if (eventX < sliderElement.getWidth() / 10f + (mMinimumStars * sliderElement.getWidth())) {
                setRating(mMinimumStars, true);
                return;
            }

            if (!isPositionInRatingView(eventX, sliderElement)) {
                continue;
            }

            float rating = RateSliderUtils.calculateRating(sliderElement, mStepSize, eventX);
    ...
    private void handleClickEvent(float eventX) {
        for (SliderElement sliderElement : mSliderElements) {
            ...
            float rating = mStepSize == 1 ? (int) sliderElement.getTag() : RateSliderUtils.calculateRating(sliderElement, mStepSize, eventX);
